...
import type { SshKeysView, SSHKeyPair } from "./api-client/validators.js";
...
export class SshKeys {
  ...
  async get(): Promise<SshKeysView> {
    return this.client.get("io.pocketenv.sandbox.getSshKeys", {
      id: this.sandboxId,
    });
  }

  async put(publicKey: string, privateKey: string): Promise<void> {
    const encryptedPrivateKey = this.publicKeyHex
      ? await encrypt(privateKey, this.publicKeyHex)
      : privateKey;

    const redacted = redactPrivateKey(privateKey);

    await this.client.post("io.pocketenv.sandbox.putSshKeys", {
      id: this.sandboxId,
  ...
  async generate(comment = "user@pocketenv"): Promise<SSHKeyPair> {
    return generateEd25519SSHKeyPair(comment);
  }
}

function redactPrivateKey(privateKey: string): string {
...
// ---- Ed25519 SSH key generation (ported from CLI) ----
...
function publicLineFromPublicKey(publicKey: Uint8Array, comment: string): string {
...
export async function generateEd25519SSHKeyPair(
  ...
  return {
    publicKey: publicLineFromPublicKey(publicKey, comment),
    privateKey: buildOpenSSHEd25519PrivateKey(publicKey, seed, comment),
  };
}
